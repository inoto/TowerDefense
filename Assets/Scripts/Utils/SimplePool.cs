...
public static class SimplePool {
	...
	class Pool {
		...
		public Pool(GameObject prefab, Transform par, int initialQty) {
			this.prefab = prefab;
	...
	static void Init (GameObject prefab, Transform parent, int qty = DEFAULT_POOL_SIZE) {
		if(pools == null) {
			pools = new Dictionary<GameObject, Pool>();
		}
		if(prefab!=null && pools.ContainsKey(prefab) == false) {
			pools[prefab] = new Pool(prefab, parent, qty);
		}
	}
